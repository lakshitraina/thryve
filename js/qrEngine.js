 ...
 * Generates 100% Camera & Phone Scannable QR Codes (SVG / Canvas / DataURL)
 */
...
(function (root, factory) {
  ...
  QR8bitByte.prototype = {
    getLength: function () {
      return encodeURI(this.data).replace(/%[0-9a-fA-F]{2}/g, 'a').length;
    },
  ...
  /**
   * Main Global Helper: generateQrSvg(text)
   * Produces crisp, 100% camera-readable standard SVG QR codes
   */
  return function generateQrSvg(text) {
    try {
      var qr = new QRCodeModel(0, 1); // Level M error correction (15%)
      qr.addData(text);
      qr.make();
      var count = qr.getModuleCount();
      var svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + count + ' ' + count + '" width="100%" height="100%" shape-rendering="crispEdges">';
      svg += '<rect width="100%" height="100%" fill="#FFFFFF"/>';
      for (var r = 0; r < count; r++) {
        for (var c = 0; c < count; c++) {
          if (qr.isDark(r, c)) {
            // Corner eyes styled in deep dark & crimson
            var isEye = (r < 7 && c < 7) || (r < 7 && c >= count - 7) || (r >= count - 7 && c < 7);
            var fill = isEye ? '#0A0F1D' : '#0F172A';
            svg += '<rect x="' + c + '" y="' + r + '" width="1" height="1" fill="' + fill + '"/>';
          }
        }
      }
      svg += '</svg>';
